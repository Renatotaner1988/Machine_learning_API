...
const postItem = async (inputPatient, inputAge, inputSex,
  inputCp, inputTrestbpd, inputChol, 
...
formData.append('trestbpd', inputTrestbpd);
...
.then((response) => response.json())
.then((data) => {
const num = data.num;  // Assumindo que o backend retorna o diagnóstico (num)

// Agora chamamos insertList aqui após o valor 'num' ser retornado.
insertList(inputPatient, inputAge, inputSex,
inputCp, inputTrestbpd, inputChol, 
inputFbs, inputRestecg, inputThalach,
inputExang, inputOldpeak, inputSlope,
inputCa, inputThal, num);
})
.catch((error) => {
console.error('Error:', error);
});
...
const newItem = async () => {
  ...
  let inputTrestbpd = document.getElementById("newTrestbpd").value;
  ...
  .then((data) => {
    ...
    } else if (isNaN(inputAge) || isNaN(inputSex) || isNaN(inputCp) || isNaN(inputTrestbpd) || isNaN(inputChol) || isNaN(inputFbs) || isNaN(inputRestecg) || isNaN(inputThalach) 
      || isNaN(inputExang) || isNaN(inputOldpeak) || isNaN(inputSlope) || isNaN(inputCa) || isNaN(inputThal) ) {
      alert("Esse(s) campo(s) precisam ser números!");
    } else {
      // Agora a chamada do insertList será feita APENAS após o POST retornar.
      postItem(inputPatient, inputAge, inputSex,
        inputCp, inputTrestbpd, inputChol, 
        inputFbs, inputRestecg, inputThalach,
        inputExang, inputOldpeak, inputSlope,
        inputCa, inputThal);
...
/*
  ...
  --------------------------------------------------------------------------------------
*/
const insertList = (namePatient, age, sex, cp, trestbps, chol, fbs, restecg, thalach, exang, oldpeak, slope, ca, thal, num) => {
  var item = [namePatient, age, sex, cp, trestbps, chol, fbs, restecg, thalach, exang, oldpeak, slope, ca, thal, num];
  var table = document.getElementById('myTable');
  ...
  document.getElementById("newTrestbpd").value = "";
